...
async function updateHeadcount(projectId: string, value: string) {
  const headcount = value === '' ? null : Number(value);
  await supabase.from('wip_projects').update({ planned_headcount: headcount }).eq('id', projectId);
}
        ...
        {activeProjects.map((p) => {
          ...
          return (
            <div key={p.id} className="card">
              ...
              <div style={{ fontSize: 11, color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                <span>
                  {new Date(p.start_date!).toLocaleDateString('en-AU')} → {new Date(p.planned_completion_date!).toLocaleDateString('en-AU')}
                </span>
                {crewOnProject.length > 0 ? (
                  <span>· {crewOnProject.join(', ')}</span>
                ) : (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <input
                      type="number"
                      min={0}
                      defaultValue={p.planned_headcount ?? ''}
                      onBlur={(e) => updateHeadcount(p.id, e.target.value)}
                      placeholder="—"
                      style={{ width: 40, padding: '2px 4px', borderRadius: 4, border: '0.5px solid var(--color-border)', fontSize: 11 }}
                    />
                  </span>
                )}
              </div>
